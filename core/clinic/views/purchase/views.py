...
from core.clinic.forms import PurchaseForm, Purchase, PurchaseDetail, Product, Provider, ProviderForm, PRODUCT_TYPE
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if action == 'add':
                ...
                queryset = Product.objects.filter().exclude(type=PRODUCT_TYPE[2][0]).exclude(id__in=ids).order_by('name')
                if len(term):
